...
def check_position_args(args):
    ...
    """
    if os.path.isfile(args.f_xyz) is not True:
        SigExit("Terminated: coordinate file not existed!\n")
    if args.f_xyz.endswith('.xyz') is not True:
...
def check_optional_arg(args):
    ...
    if args.guess != 'atom' and \
       os.path.isfile(args.guess) is not True:
    ...
    if args._method == 'losc':
        ...
        if len(args.window.split()) > 2:
            SigExit("Terminated: arg[window] at most 2 nums\n")
                ...
                and args.window != '0':
            SigExit(
                "Terminated: arg[window] has to be 0 to disable LOEnergy\n")
...
def write_xyz_g09(f_inp, f_xyz):
    ...
    for line in f2:
        ...
        if len(line_split) > 0:  # skip the emmty line
            if line_split[0].startswith('#') is not True:
                count += 1
            # start write coordinates
            if count >= 3 and (not line_split[0].startswith('#')):
                ...
                line = '{:<3} {:<10f} {:<10f} {:<10f}'\
                    .format(line_split[0], float(line_split[1]),
